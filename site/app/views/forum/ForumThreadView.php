            ...
            foreach($threads as $thread){
                ...
                if(!isset($_REQUEST["thread_id"]) && !$used_active){
                    ...
                    if($thread["pinned"])
						...
						//replace tags from displaying in sidebar$first_post_content = str_replace("[/code]", "", str_replace("[code]", "", strip_tags($first_post["content"])));
...
HTML;
					...
					$first_post_id = 1;
					$order_array = array();
					$reply_level_array = array();foreach($posts as $post){if($thread_id == -1) {
...
HTML;
	...
	public function createPost($thread_id, $post, $function_date, $title_html, $first, $reply_level){
		...
		$post_id = $post["id"];
						...
						$full_name = $this->core->getQueries()->getDisplayUserNameFromUserId($post["author_user_id"]);$first_name = htmlentities(trim($full_name["first_name"]), ENT_QUOTES | ENT_HTML5, 'UTF-8');
